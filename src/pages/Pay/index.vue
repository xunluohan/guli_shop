    ...
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          ...
          <a href="javascript:;" class="btn" @click.prevent="paySuccess"
            >立即支付</a
          >
        </div>
        <div class="otherpay">
  ...
  methods: {
    ...
    async paySuccess() {
      ...
      try {
        ...
        this.$alert(`<img src="${imgUrl}">`, "HTML 片段", {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              // if (!this.payStatus) {
              //   this.$message.warning("支付成功后,自动跳转到支付成功页面");
              // }
              // 关闭定时器
              ...
              this.$message.success("支付遇到问题,请联系尚硅谷小姐姐");
              // 关闭定时器
              clearInterval(this.timer);
              this.timer = null;
              // 关闭窗口
        ...
        if (!this.timer) {
          this.timer = setInterval(async () => {
            try {
              ...
              this.$message.error(error.message + "获取支付状态失败");
            }
          }, 2000);
        }
      } catch (error) {
