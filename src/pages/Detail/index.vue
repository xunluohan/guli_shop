            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  v-model="skuNum"
                  @change="
                    skuNum >= 1 ? (skuNum = $event.target.value) : (skuNum = 1)
                  "
                />
...
export default {
  ...
  methods: {
    ...
    // 点击切换商品属性 高亮 (排它)
    changeAcitve(spuSaleAttrValueList, spuSaleAttrValue) {
      spuSaleAttrValueList.forEach((item) => (item.isChecked = "0"));
      spuSaleAttrValue.isChecked = "1";
    },
    // 点击添加购物车 或 更新购物车
    async addOrUpdateShopCart() {
      // 发送请求添加购物车,添加成功后,跳转到购物车页面,添加失败,则给个提示
      try {
        ...
        // 将skuInfo 序列化,临时存储到 sessionStorage 上
