  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="address in addressInfoList"
        :key="address.id"
      >
        <span
          class="username"
        ...
        >
        <p @click="changeDefaultAddress(addressInfoList, address)">
...
export default {
  ...
  methods: {
    ...
    async getAddressInfoList() {
      this.$store.dispatch("getAddressInfoList");
    },
    // 切换默认地址
    changeDefaultAddress(addressInfoList, address) {
      addressInfoList.forEach((item) => (item.isDefault = "0"));
      address.isDefault = "1";
    },
    async pay() {
      // 获取交易编号
      let orderNo = this.tradeInfo.tradeNo;
      // 获取交易信息
      let orderInfo = {
      ...
      // 发请求
      try {
        const result = await this.$API.reqSubmitOrder(orderNo,orderInfo)
        if(result.code === 200){
          alert('提交订单成功,即将跳转到支付页面')
